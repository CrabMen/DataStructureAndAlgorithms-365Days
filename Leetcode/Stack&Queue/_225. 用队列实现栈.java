 ...
 * empty 这些操作是合法的。 你所使用的语言也许不支持队列。 你可以使用 list 或者 deque（双端队列）来模拟一个队列 ,
...
/**
 * 方法一: 双队列实现栈,辅助queue,q2,每次添加之前都保证该q2为空,然后将q1中的元素依次poll后添加到q1中
 */
class MyStack {
 ...
 * 方法二:单队列实现栈
 * 
 * 提交的时候记得将 MyStack1 修改为MyStack
 *
 * 通过对当前的queue进行(size - 1) 次的删除和添加,从而完成队列翻转的操作
 * 翻转的时机是在push还是pop中,都可以
 */
